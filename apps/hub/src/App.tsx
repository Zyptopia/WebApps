...
export default function App() {
  ...
  const [activeReactions, setActiveReactions] = useState<
    Record<string, { type: keyof typeof REACTION_EMOJI; key: string }>
  >({});

  // lobby message
  const [lobbyMsg, setLobbyMsg] = useState<string | null>(null);
  const showLobbyMsg = (m: string) => {
    setLobbyMsg(m);
    window.setTimeout(() => setLobbyMsg(null), 3000);
  };
  ...
  useEffect(() => {
    return client.onReactions(
      (
        events: Array<{ playerId: string; type: keyof typeof REACTION_EMOJI; createdAt: number }>
      ) => {
        const latestPerPlayer: Record<string, { type: keyof typeof REACTION_EMOJI; at: number }> =
          {};
        for (const ev of events) {
          const cur = latestPerPlayer[ev.playerId];
          if (!cur || ev.createdAt > cur.at)
            latestPerPlayer[ev.playerId] = { type: ev.type as any, at: ev.createdAt };
        }
    ...
    return client.onModeration((res: any) => {
      if (!res.ok) {
        if (res.reason === 'TOO_LONG') setWarn('Message too long (max 160).');
          ...
          setWarn('Chat delay is on…');
          setCooldownMs(left);
        } else setWarn('Message blocked.');
      } else {
        setCooldownMs(0);
        setWarn(res.replaced ? 'Profanity/links adjusted.' : null);
      }
    });
  }, [client]);
  ...
  // countdown ticker
  useEffect(() => {
    if (!room?.epochStart || room?.status !== 'starting') {
      setCountdownMs(0);
      return;
    }
      ...
      try {
        ...
        if (codeFromLink && /^[A-Z0-9]{4}$/.test(codeFromLink)) {
          ...
          try {
            ...
            if (msg.includes('ERR_CODE_NOT_FOUND'))
              setError('Code not found. Check the 4-letter code and try again.');
            else if (msg.toLowerCase().includes('room not found'))
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      if (msg.includes('ERR_PERMISSION_DENIED'))
        setError('Permission denied by database rules. Check Anonymous Auth + rules.');
      else if (msg.includes('ERR_ALLOCATE_CODE'))
        setError('Could not allocate a join code. Try again.');
      else if (msg.includes('ERR_CODE_WRITE'))
        setError('Could not reserve join code — rules forbid writes to /codes.');
  ...
  const joinRoom = async () => {
    ...
    if (entered.length !== 4) {
      setError('Enter a 4-character code (e.g., AB12).');
      return;
    }
    try {
      await client.joinRoomByCode({ code: entered, name: nickname, avatar });
      setView('lobby');
      setError(null);
    } catch (e: any) {
      const msg = String(e?.message || e);
  ...
  const everyoneReady =
    players.length >= 2 && readyIds.length >= 2 && readyIds.length === players.length;
  ...
  const startCountdown = async () => {
    if (!amHost) {
      showLobbyMsg('Only the host can start the game.');
      return;
    }
    if (players.length < 2) {
      showLobbyMsg('Need at least 2 players to start.');
      return;
    }
    if (!everyoneReady) {
      showLobbyMsg('Everyone must be Ready before starting.');
      return;
    }
    try {
  ...
  const shareCode = async () => {
    ...
    try {
      if ((navigator as any).share)
        await (navigator as any).share({ title: 'Creative Hub', text, url: link });
      else {
        await navigator.clipboard.writeText(text);
        setWarn('Join link copied to clipboard.');
      }
    } catch {}
  };
  ...
  const sendReaction = async (type: keyof typeof REACTION_EMOJI) => {
    ...
    try {
      await client.sendReaction(type as any);
    } catch (e) {
  ...
  const mutePlayer = async (pid: string) => {
    try {
      await client.shadowMute(pid, 5);
      showLobbyMsg('Player muted (5 min).');
    } catch {
      showLobbyMsg('Could not mute player.');
    }
  };
  const unmutePlayer = async (pid: string) => {
    try {
      await client.shadowUnmute(pid);
      showLobbyMsg('Player unmuted.');
    } catch {
      showLobbyMsg('Could not unmute player.');
    }
  };
  ...
  const renderCountdown = () => {
    ...
    return (
      <div className="small" style={{ fontSize: 18, fontWeight: 600 }}>
        {sec > 0 ? `Starting in ${sec}…` : 'GO!'}
      </div>
    );
  };
                      ...
                      return (
                        ...
                        >
                          <div className="avatar">
                            {renderAvatar({ kind: 'preset', id } as any, AVATAR_SIZE)}
                          </div>
                        </button>
                      );
                    })}
        ...
        <div className="card">
          ...
          >
            <div>
              ...
              <div className="code" style={{ fontSize: 24 }}>
                {room?.joinCode ?? '----'}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <Button type="button" onClick={shareCode} title="Copy/share Join link">
                Share code
              </Button>
              {amHost && (
                <Button
          ...
          <div className="row" style={{ alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
              <Button type="button" onClick={toggleReady}>
                {isSelfReady ? 'Unready' : 'Ready'}
              </Button>
              <Button type="button" onClick={startCountdown} title="Host: start countdown">
                Start in 3s
              </Button>
            </div>
          </div>
          ...
          {lobbyMsg && (
            <div className="small" role="status" style={{ marginTop: 6, opacity: 0.9 }}>
              {lobbyMsg}
            </div>
          )}
              ...
              return (
                ...
                >
                  ...
                  <div style={reactionSlotStyle}>
                    {active && (
                      <div style={reactionChipStyle} aria-hidden>
                        {REACTION_EMOJI[active.type]}
                      </div>
                    )}
                  </div>
                    ...
                    <div>
                      <div>
                        ...
                        {isReady && (
                          <span
                            className="small"
                            style={{ marginLeft: 8, color: 'var(--ok, #16a34a)' }}
                          >
                            • Ready
                          </span>
                        )}
                        ...
                        {isMuted && (
                          <span className="small" style={{ marginLeft: 8, color: '#f59e0b' }}>
                            • Muted
                          </span>
                        )}
                      </div>
          ...
          <div className="row" style={{ gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
            <Button type="button" onClick={() => sendReaction('wave')} style={iconBtn} title="Wave">
              {REACTION_EMOJI.wave}
            </Button>
            <Button type="button" onClick={() => sendReaction('clap')} style={iconBtn} title="Clap">
              {REACTION_EMOJI.clap}
            </Button>
            <Button
              type="button"
              ...
              title="Laugh"
            >
              {REACTION_EMOJI.laugh}
            </Button>
            <Button type="button" onClick={() => sendReaction('wow')} style={iconBtn} title="Wow">
              {REACTION_EMOJI.wow}
            </Button>
            <Button type="button" onClick={() => sendReaction('nope')} style={iconBtn} title="Nope">
              {REACTION_EMOJI.nope}
            </Button>
          </div>
          ...
          <div className="row stretch" style={{ marginTop: 8 }}>
            <Input
              ...
              placeholder={
                cooldownMs > 0 ? `Chat delay: ${(cooldownMs / 1000).toFixed(1)}s` : 'Say hi'
              }
              maxLength={160}
              aria-invalid={!!warn}
            ...
            <Button type="button" onClick={sendChat} disabled={!canSend}>
              {cooldownMs > 0 ? 'Wait…' : 'Send'}
            </Button>
          </div>
          {warn && (
            <div id="chat-warn" className="small" style={{ marginTop: 6, opacity: 0.8 }}>
              {cooldownMs > 0
                ? `Chat delay active — ${(cooldownMs / 1000).toFixed(1)}s remaining.`
                : warn}
            </div>
          )}
        </div>
      ...
      >
        <form method="dialog" style={{ margin: 0 }}>
          ...
          <div className="row" style={{ marginTop: 16, justifyContent: 'flex-end', gap: 8 }}>
            ...
            <Button type="button" onClick={saveSettings} disabled={!amHost}>
              Save
            </Button>
          </div>
        </form>
