...
const pickPresetId = () =>
  DEFAULT_PRESETS[Math.floor(Math.random() * DEFAULT_PRESETS.length)];
...
export class RoomClient {
  ...
  currentIsHost() {
    return !!this.room && this.room.hostId === this.guestId;
  }
  ...
  onPlayers(cb: (players: Player[]) => void) {
    ...
    if (this.players.length) cb(this.players);
    return () => {
      this.playersListeners = this.playersListeners.filter((f) => f !== cb);
    };
  }
  onRoomMeta(cb: (room: Room | null) => void) {
    this.roomListeners.push(cb);
    if (this.room) cb(this.room);
    return () => {
      this.roomListeners = this.roomListeners.filter((f) => f !== cb);
    };
  }
  onChat(cb: (msgs: ChatMessage[]) => void) {
    this.chatListeners.push(cb);
    if (this.chat.length) cb(this.chat);
    return () => {
      this.chatListeners = this.chatListeners.filter((f) => f !== cb);
    };
  }
  onModeration(cb: (m: ModerateResult) => void) {
    this.moderationListeners.push(cb);
    return () => {
      this.moderationListeners = this.moderationListeners.filter((f) => f !== cb);
    };
  }

  onReady(cb: (ready: Set<string>) => void) {
    this.readyListeners.push(cb);
    if (this.readyIds.size) cb(new Set(this.readyIds));
    return () => {
      this.readyListeners = this.readyListeners.filter((f) => f !== cb);
    };
  }
  onReactions(cb: (events: ReactionEvent[]) => void) {
    this.reactionListeners.push(cb);
    if (this.reactions.length) cb(this.reactions.slice());
    return () => {
      this.reactionListeners = this.reactionListeners.filter((f) => f !== cb);
    };
  }
  ...
  private notifyRoom() {
    this.roomListeners.forEach((cb) => cb(this.room));
  }
  private notifyChat() {
    this.chatListeners.forEach((cb) => cb(this.chat));
  }
  private notifyModeration(res: ModerateResult) {
    this.moderationListeners.forEach((cb) => cb(res));
  }
  private notifyReady() {
    const snapshot = new Set(this.readyIds);
    this.readyListeners.forEach((cb) => cb(snapshot));
  }
  private notifyReactions() {
    const copy = this.reactions.slice();
    this.reactionListeners.forEach((cb) => cb(copy));
  }
  ...
  private attachSubscriptions() {
    ...
    const u1 = onValue(playersRef, (snap) => {
      const val = snap.val() || {};
      const list: Player[] = Object.values(val);
      this.players = list.sort((a, b) => a.name.localeCompare(b.name));
      this.notifyPlayers();
    });
    const u2 = onValue(roomRef, (snap) => {
      this.room = snap.val() || null;
      this.notifyRoom();
    });
    const u3 = onValue(chatRef, (snap) => {
      const val = snap.val() || {};
      const list: ChatMessage[] = Object.entries(val).map(([id, v]: any) => ({ id, ...v }));
      this.serverChat = list.sort((a, b) => a.createdAt - b.createdAt);
      this.rebuildChatWithEchoes();
    });
    const u4 = onValue(readyRef, (snap) => {
      const val = (snap.val() || {}) as Record<string, any>;
      const ids = Object.keys(val).filter((k) => !!val[k]);
      this.readyIds = new Set(ids);
      this.notifyReady();
    });
    const u5 = onValue(reactRef, (snap) => {
      const val = (snap.val() || {}) as Record<string, any>;
      const list: ReactionEvent[] = Object.entries(val).map(([id, v]: any) => ({ id, ...v }));
  ...
  private rebuildChatWithEchoes() {
    ...
    const merged = [...this.serverChat, ...this.localEchoes].sort(
  ...
  private async startPresence(name: string, avatar?: Avatar | null) {
    ...
    const now = Date.now();
    const assignedAvatar: Avatar | undefined = avatar ?? {
      kind: 'preset',
      id: pickPresetId(),
    };
    const role: Player['role'] =
      this.room?.hostId === this.guestId ? 'host' : 'player';
    const p: Player = {
      id: this.guestId,
      name,
      role,
    ...
    const presRef = ref(
      this.fb.db,
      `rooms/${this.roomId}/presence/${this.guestId}`,
    );

    set(pRef, p);
    set(presRef, { lastSeen: now });

    this.discPlayer = onDisconnect(pRef);
    this.discPlayer.remove();
    this.discPresence = onDisconnect(presRef);
    this.discPresence.remove();

    if (this.heart) clearInterval(this.heart);
    this.heart = setInterval(() => {
      const t = Date.now();
      update(pRef, { lastSeen: t });
      set(presRef, { lastSeen: t });
    }, HEARTBEAT_MS);
  ...
  private async allocateCode(roomId: string): Promise<string> {
    for (let i = 0; i < 32; i++) {
      ...
      try {
        const res = await runTransaction(codeRef, (cur) =>
          cur ? cur : { roomId, createdAt: Date.now() },
        );
  ...
  async createRoom(input: CreateRoomInput) {
    ...
    try {
      ...
      const assignedAvatar: Avatar | undefined =
        input.avatar ?? { kind: 'preset', id: pickPresetId() };
      const player: Player = {
        id: this.guestId,
      ...
      // Write paths explicitly to satisfy strict rules
      await set(ref(this.fb.db, `rooms/${roomId}/meta`), room);
      await set(
        ref(this.fb.db, `rooms/${roomId}/players/${this.guestId}`),
        player,
      );
      ...
      if (String(code).includes('ERR_ALLOCATE_CODE'))
        throw new Error('ERR_ALLOCATE_CODE');
  ...
  async sendText(text: string) {
    ...
    // NOTE: read allowLinks with an 'any' cast so this compiles
    // even if your Room.options type doesn't declare it yet.
    const modOptions = {
      slowModeMs: this.room?.options?.slowModeMs,
      allowLinks: ((this.room as any)?.options?.allowLinks === true),
    } as any;

    const res = await this.moderation.moderate({
      roomId: this.roomId!,
      playerId: this.guestId,
      text,
      options: modOptions,
    });
    if (!res.ok) {
      this.notifyModeration(res);
      return;
    }

    let clean = (res.text || '').slice(0, CHAT_MAX_LEN).trim();
    if (!clean) {
      this.notifyModeration({ ok: false, reason: 'EMPTY' });
      return;
    }

    const name =
      this.players.find((p) => p.id === this.guestId)?.name ?? 'Guest';
  ...
  async setReady(flag?: boolean) {
    ...
    const nodeRef = ref(
      this.fb.db,
      `rooms/${this.roomId}/ready/${this.guestId}`,
    );
  ...
  async hostStartCountdown(seconds = 3) {
    ...
    const playersSnap = await get(
      ref(this.fb.db, `rooms/${this.roomId}/players`),
    );
    const readySnap = await get(ref(this.fb.db, `rooms/${this.roomId}/ready`));
    const playersObj = (playersSnap.val() || {}) as Record<string, Player>;
    const readyObj = (readySnap.val() || {}) as Record<string, true>;

    const presentIds = Object.keys(playersObj);
    if (presentIds.length < 2) throw new Error('ERR_TOO_FEW_PLAYERS');
    for (const id of presentIds) if (!readyObj[id]) throw new Error('ERR_NOT_ALL_READY');
  ...
  async sendReaction(type: ReactionType) {
    await this.waitAuth();
    if (!this.roomId) throw new Error('Not in room');
    if (this.room?.options?.reactions === false) return;
    if (!REACTION_TYPES.includes(type)) throw new Error('ERR_BAD_REACTION');

    const now = Date.now();
    if (now - this.lastReactionAt < 2000) return;
  ...
  /** Host-only: update room options (slowModeMs, allowLinks, reactions). */
  async setRoomOptions(
    opts: Partial<{ slowModeMs: number; allowLinks: boolean; reactions: boolean; spectators: boolean }>,
  ) {
    await this.waitAuth();
    if (!this.roomId || !this.room) throw new Error('Not in room');
    if (this.room.hostId !== this.guestId) throw new Error('ERR_NOT_HOST');

    const base = `rooms/${this.roomId}/meta/options`;
    const tasks: Promise<any>[] = [];
    if (typeof opts.slowModeMs === 'number')
      tasks.push(
        set(
          ref(this.fb.db, `${base}/slowModeMs`),
          Math.max(0, Math.min(10000, Math.floor(opts.slowModeMs))),
        ),
      );
    if (typeof opts.allowLinks === 'boolean')
      tasks.push(set(ref(this.fb.db, `${base}/allowLinks`), !!opts.allowLinks));
    if (typeof opts.reactions === 'boolean')
      tasks.push(set(ref(this.fb.db, `${base}/reactions`), !!opts.reactions));
    if (typeof opts.spectators === 'boolean')
      tasks.push(set(ref(this.fb.db, `${base}/spectators`), !!opts.spectators));

    if (tasks.length) await Promise.all(tasks);
  }

  /** Host-only: mute a player for durationMs (shadow mute). */
  async mutePlayer(playerId: string, durationMs: number) {
    await this.waitAuth();
    if (!this.roomId || !this.room) throw new Error('Not in room');
    if (this.room.hostId !== this.guestId) throw new Error('ERR_NOT_HOST');
    const until = Date.now() + Math.max(0, durationMs);
    await update(
      ref(this.fb.db, `rooms/${this.roomId}/players/${playerId}`),
      { mutedUntil: until },
    );
  }

  /** Host-only: clear mute. */
  ...
  /** Host-only: reset a player's avatar to a random preset (anti-abuse). */
  async resetPlayerAvatar(playerId: string) {
    await this.waitAuth();
    if (!this.roomId || !this.room) throw new Error('Not in room');
    if (this.room.hostId !== this.guestId) throw new Error('ERR_NOT_HOST');
    const preset: Avatar = { kind: 'preset', id: pickPresetId() };
    await update(
      ref(this.fb.db, `rooms/${this.roomId}/players/${playerId}`),
      { avatar: preset as any },
    );
  }
  ...
  async leaveRoom() {
    if (!this.roomId) return;
    try {
      await this.discPlayer?.cancel();
    } catch {}
    try {
      await this.discPresence?.cancel();
    } catch {}
    try {
      if (this.presenceRef) await set(this.presenceRef, null as any);
    } catch {}
    try {
      if (this.playerRef) await set(this.playerRef, null as any);
    } catch {}

    this.unsubFns.forEach((f) => f());
    this.unsubFns = [];
    if (this.heart) clearInterval(this.heart);
