...
export class RoomClient {
  ...
  private attachSubscriptions() {
    ...
    const u3 = onValue(chatRef, (snap) => {
      const val = snap.val() || {};
      const list: ChatMessage[] = Object.entries(val).map(([id, v]: any) => ({
        id,
        ...(v as any),
      }));
    ...
    const u5 = onValue(reactRef, async (snap) => {
      const val = (snap.val() || {}) as Record<string, any>;
      const list: ReactionEvent[] = Object.entries(val).map(([id, v]: any) => ({
        id,
        ...(v as any),
      }));
      ...
      const LIMIT = 50;
      const now = Date.now();
      const amHost = this.room && this.room.hostId === this.guestId;
      if (
        amHost &&
        !this.cleaningReactions &&
        list.length > LIMIT &&
        now - this.lastReactionCleanupAt > 10_000
      ) {
        this.cleaningReactions = true;
        this.lastReactionCleanupAt = now;
  ...
  async createRoom(input: CreateRoomInput) {
    ...
    const assignedAvatar: Avatar | undefined =
      input.avatar ?? ({ kind: 'preset', id: pickPresetId() } as Avatar);
    const player: Player = {
      id: this.guestId,
  ...
  async sendText(text: string) {
    ...
    const chatDelay =
      (this.room as any)?.options?.chatDelayMs ?? (this.room as any)?.options?.slowModeMs ?? 0;
  ...
  async setReady(flag?: boolean) {
    ...
    if (shouldSet) await set(ref(this.fb.db, path), true);
  ...
  async updateOptions(partial: {
    ...
    if (
      Object.prototype.hasOwnProperty.call(partial, 'chatDelayMs') ||
      Object.prototype.hasOwnProperty.call(partial, 'slowModeMs')
    ) {
    ...
    if (Object.prototype.hasOwnProperty.call(partial, 'reactionsEnabled'))
      patch.reactionsEnabled = !!partial.reactionsEnabled;
    if (Object.prototype.hasOwnProperty.call(partial, 'spectators'))
      patch.spectators = !!partial.spectators;
  ...
  async leaveRoom() {
    ...
    try {
      if (this.playerRef) await set(this.playerRef, null);
    } catch {}
